        ...
        try {
            ...
            if (platform.equalsIgnoreCase("Remote")) {
                if (browser.equalsIgnoreCase("firefox")) {
                    ...
                    driver = new RemoteWebDriver(new URL("http://10.0.2.15:4444/wd/hub"), cap);
                }
                ...
                String URL = "http://" + saucelabaccountname + ":" + saucelabaccesskey
                ...
                if (_OS_MAC.compareToIgnoreCase(OS) == 0) {
                    ...
                    String Macphantomjspath = System.getProperty("user.dir")
                            + "/src/test/resources/phantomjs-2.1.1-macosx/bin/phantomjs";
                    caps.setCapability(PhantomJSDriverService.PHANTOMJS_EXECUTABLE_PATH_PROPERTY, Macphantomjspath);
                } else if (_OS_LINUX.compareToIgnoreCase(OS) == 0) {
                    caps.setJavascriptEnabled(true);
                    caps.setCapability("cssSelectorsEnabled", false);
                    caps.setCapability("applicationCacheEnabled", true);
                    caps.setCapability("acceptSslCerts", true);
                    String Linuxphantomjspath = System.getProperty("user.dir")
                            + "/src/test/resources/phantomjs-2.1.1-linux-x86_64/bin/phantomjs";
                    caps.setCapability(PhantomJSDriverService.PHANTOMJS_EXECUTABLE_PATH_PROPERTY, Linuxphantomjspath);
                } else if (_OS_WINDOWS.compareToIgnoreCase(OS) == 0) {

                    caps.setJavascriptEnabled(true);
                    caps.setCapability("cssSelectorsEnabled", false);
                    caps.setCapability("applicationCacheEnabled", true);
                    caps.setCapability("acceptSslCerts", true);
                    String windowsphantomjspath = System.getProperty("user.dir")
                            + "/src/test/resources/phantomjs-2.1.1-windows/bin/phantomjs.exe";
